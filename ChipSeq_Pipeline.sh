...
if [ -z "$fastqdir" ] && [ -z "$refgenome" ] ; then
    echo -e "\n\e[1;96mChIP-Seq Pipeline\e[1;m"
    echo -e  "\e[35m\n-f is not supplied\e[1;m    -> \e[94mPlease input the directory containing fastq files \e[1;m"
...
for dir in "$fastqdir"/*; do
    ...
    ##Get sample name and output dir
    ...
    echo -e "$(date)\tSorting SAM file by name and converting to BAM using samtools." >> "$sampleDir"/analysis.log
    ...
    ##Get stats after extracting primary reads.
    ...
    ### Automate peak calling mode (MACS3) based on Encode guide ###
...
## MultiQC to get QC of all steps
source activate ngsmo
